...
nonisolated enum StrokeGeometry {
    ...
    static func tessellate(
        ...
        if clean.count == 1 || totalLength(clean) < 1e-9 {
            ...
            return disc(center: clean[0].center, radius: max(w, minWidth) * 0.5, color: color, segments: max(capSegments * 2, 16))
        }

        // 1. Catmull-Rom -> Bézier 分段 + 自适应展平（含宽度插值）
        var samples: [(CGPoint, CGFloat)] = []
        samples.reserveCapacity(clean.count * 4)
        samples.append((clean[0].center, clean[0].width))
        for i in 0..<(clean.count - 1) {
            ...
            let w1 = clean[i].width
            let w2 = clean[i + 1].width
            flattenCubic(
                p0: p1,
                c1: p1 + (p2 - p0) / 6,
                c2: p2 - (p3 - p1) / 6,
                p3: p2,
                w0: w1, w1: w2, t0: 0, t1: 1,
        ...
        for i in 0..<n {
            let prev = samples[max(0, i - 1)].0
            let next = samples[min(n - 1, i + 1)].0
            var t = next - prev
            ...
            let hw = max(samples[i].1, minWidth) * 0.5
            vertices.append(StrokeVertex(position: samples[i].0 + normal * hw, color: color))
            vertices.append(StrokeVertex(position: samples[i].0 - normal * hw, color: color))
        }
        for i in 0..<(n - 1) {
            let b = UInt32(i * 2)
        ...
        appendCap(
            center: samples[0].0, radius: max(samples[0].1, minWidth) * 0.5,
            facing: -t0, color: color, segments: capSegments,
            vertices: &vertices, indices: &indices
        )
        appendCap(
            center: samples[n - 1].0, radius: max(samples[n - 1].1, minWidth) * 0.5,
            facing: t1, color: color, segments: capSegments,
            vertices: &vertices, indices: &indices
        )
    ...
    private static func disc(center: CGPoint, radius: CGFloat, color: RGBA, segments: Int) -> StrokeMesh {
        var vertices: [StrokeVertex] = [StrokeVertex(position: center, color: color)]
        var indices: [UInt32] = []
        for i in 0...segments {
    ...
    private static func flattenCubic(
        ...
        w0: CGFloat, w1: CGFloat, t0: CGFloat, t1: CGFloat,
        tolerance: CGFloat,
        depth: Int,
        into out: inout [(CGPoint, CGFloat)]
    ) {
        // 平直测试：控制点到弦的最大距离
        ...
        let mid = (n1 + n2) / 2
        let tm = (t0 + t1) / 2
        flattenCubic(p0: p0, c1: m1, c2: n1, p3: mid, w0: w0, w1: w1, t0: t0, t1: tm, tolerance: tolerance, depth: depth + 1, into: &out)
        flattenCubic(p0: mid, c1: n2, c2: m3, p3: p3, w0: w0, w1: w1, t0: tm, t1: t1, tolerance: tolerance, depth: depth + 1, into: &out)
    }
    ...
    private static func appendCap(
        ...
        vertices.append(StrokeVertex(position: center, color: color))
    ...
    private static func cleaned(_ spine: [SpinePoint]) -> [SpinePoint] {
        ...
        for s in spine {
            ...
            out.append(SpinePoint(center: s.center, width: w))
        }
        return out
    }
    ...
    private static func tangentOf(_ samples: [(CGPoint, CGFloat)], at i: Int) -> CGPoint {
        let n = samples.count
        let prev = samples[max(0, i - 1)].0
        let next = samples[min(n - 1, i + 1)].0
        var t = next - prev
